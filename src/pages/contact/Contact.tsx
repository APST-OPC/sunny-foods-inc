...
const Contact = (): ReactElement => {
  ...
  const onSubmit: SubmitHandler<IContactForm> = (data: IContactForm) => {
    mutate(data, {
      ...
      onError: (error) => {
        toast.custom((t) => (
          <Toast t={t} icon={() => <FaCircleExclamation />} type="error" message={error.message} />
        ));
      },
    });
  };
    ...
    <main className="container mx-auto px-5 py-20 md:space-y-20 md:px-0">
      <header className="hero space-y-5">
        <section className="hero-content flex-col items-start">
          <p className="text-error font-extrabold tracking-widest uppercase">Contact Us</p>
          ...
          <div className="flex flex-col items-center gap-5 md:flex-row">
            ...
            <p className="md:w-1/3">
              Whether you need assistance with our services, have inquiries, or want to share your
              feedback, our team is always ready to support you every step of the way.
            </p>
          </div>
        </section>
      </header>
            ...
            <div className="grid gap-2.5 md:grid-cols-2">
              {contactBtns.map(({ contactCTA, contactIcon, openWindow }, ids) => (
                <div key={ids} className="flex items-center gap-2.5">
                  <button className="btn btn-error btn-circle btn-xl" onClick={openWindow}>
